...
class AuthManager:
    ...
    def _enforce_google_allowlist(self, user: User) -> None:
        """Block non-allowlisted Google users even if they somehow reach token issuance."""
        ...
        if allowed_emails and user_email not in allowed_emails:
            logger.warning(f"Rejected JWT issuance for unauthorized Google user: {user_email}")
            raise InvalidTokenException(detail="This Google account is not authorized to access this backend")

    def create_token(self, user: User) -> str:
        """Create a JWT token for authenticated user."""
        self._enforce_google_allowlist(user)
