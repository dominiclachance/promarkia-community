...
from .deps import cleanup_managers, get_db, get_manager_status, init_auth_manager, init_managers, register_auth_dependencies
from .initialization import AppInitializer
from .routes import gallery, mcp, runs, sessions, settingsroute, teams, validation, workflows, ws

# Importing the local models before database initialization registers their
# tables in SQLModel metadata. The router is mounted below under /api/local.
from promarkia_local.router import router as local_router, seed_local_catalog
from promarkia_local.scheduler import local_scheduler
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        # Seed only non-secret local defaults. Squad definitions are imported
        # separately from sanitized configuration files.
        from .deps import get_db as get_database_manager

        seed_local_catalog(await get_database_manager())
...
async def health_check(db=Depends(get_db)):
    """API health check endpoint with manager + database connectivity checks."""
