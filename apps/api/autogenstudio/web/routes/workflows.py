# /workflows routes
import asyncio
import json  
import uuid
from datetime import datetime
from typing import Dict, Any, List

from autogen_core import ComponentModel
from fastapi import APIRouter, Depends, HTTPException, Request, WebSocket, WebSocketDisconnect
from loguru import logger
from pydantic import BaseModel

from ...datamodel import WorkflowDB
from ...workflow.core import Workflow, WorkflowRunner, WorkflowConfig
from ...workflow.defaults import get_default_steps, get_default_workflows
from ..auth.dependencies import get_ws_auth_manager, get_current_user
from ..auth.models import User
from ..auth.wsauth import WebSocketAuthHandler
from ..deps import get_db
from ...mcp.utils import serialize_for_json
from .auth_utils import get_authenticated_user_id

router = APIRouter()


class CreateWorkflowRequest(BaseModel):
    name: str
    description: str = ""
    config: ComponentModel
    tags: list[str] = []


class UpdateWorkflowRequest(BaseModel):
    name: str | None = None
    description: str | None = None
    config: ComponentModel | None = None


class CreateWorkflowRunRequest(BaseModel):
    workflow_id: int | None = None
    workflow_config: Dict[str, Any] | None = None


# ==================== REST API Routes ====================

@router.get("/workflows")
async def list_workflows(
    request: Request,
    current_user: User = Depends(get_current_user),
    db=Depends(get_db)
) -> Dict[str, Any]:
    """List all workflows with optional filters"""
    try:
        user_id = get_authenticated_user_id(request)
        workflows = db.get(WorkflowDB, filters={"user_id": user_id})
        user_workflows = workflows.data or []
        
        # Return empty list if user has no workflows (no auto-creation)
        
        return {"status": True, "data": user_workflows}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.post("/workflows")
async def create_workflow(
    request: Request,
    body: CreateWorkflowRequest,
    db=Depends(get_db)
) -> Dict:
    """Create a new workflow"""
    try:
        user_id = get_authenticated_user_id(request)
        workflow = db.upsert(
            WorkflowDB( 
                config=body.config.model_dump(), 
                user_id=user_id, 
            ),
            return_json=False,
        )
        return {"status": workflow.status, "data": {"workflow_id": workflow.data.id}}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/workflows/{workflow_id}")
async def get_workflow(workflow_id: int, request: Request, db=Depends(get_db)) -> Dict:
    """Get workflow details"""
    try:
        user_id = get_authenticated_user_id(request)
        logger.info(f"Getting workflow {workflow_id} for user {user_id}")
        workflow = db.get(WorkflowDB, filters={"id": workflow_id, "user_id": user_id}, return_json=False)
        logger.info(f"Workflow query result: status={workflow.status}, data_count={len(workflow.data) if workflow.data else 0}")
        if not workflow.status or not workflow.data:
            raise HTTPException(status_code=404, detail="Workflow not found")
        return {"status": True, "data": workflow.data[0]}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting workflow {workflow_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.put("/workflows/{workflow_id}")
async def update_workflow(
    workflow_id: int, body: UpdateWorkflowRequest, request: Request, db=Depends(get_db)
) -> Dict:
    """Update workflow"""
    try:
        user_id = get_authenticated_user_id(request)
        existing = db.get(WorkflowDB, filters={"id": workflow_id, "user_id": user_id}, return_json=False)
        if not existing.status or not existing.data:
            raise HTTPException(status_code=404, detail="Workflow not found")
        
        workflow = existing.data[0]
        
        # WorkflowDB only has `config` (dict) - name/description live inside config
        if body.config is not None:
            next_config = body.config.model_dump()
            if isinstance(next_config, dict) and "metadata" not in next_config:
                next_config["metadata"] = {
                    "name": next_config.get("label") or (workflow.config or {}).get("label") or "Workflow",
                    "description": next_config.get("description") or (workflow.config or {}).get("description") or "",
                }
            workflow.config = next_config
        
        # Apply name/description overrides into the config dict
        if body.name is not None and isinstance(workflow.config, dict):
            workflow.config["label"] = body.name
            if "metadata" in workflow.config:
                workflow.config["metadata"]["name"] = body.name
        if body.description is not None and isinstance(workflow.config, dict):
            workflow.config["description"] = body.description
            if "metadata" in workflow.config:
                workflow.config["metadata"]["description"] = body.description
        updated = db.upsert(workflow, return_json=False)
        return {"status": updated.status, "data": updated.data}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.delete("/workflows/{workflow_id}")
async def delete_workflow(workflow_id: int, request: Request, db=Depends(get_db)) -> Dict:
    """Delete workflow (hard delete)"""
    try:
        user_id = get_authenticated_user_id(request)
        workflow = db.get(WorkflowDB, filters={"id": workflow_id, "user_id": user_id}, return_json=False)
        if not workflow.status or not workflow.data:
            raise HTTPException(status_code=404, detail="Workflow not found")
        
        result = db.delete(WorkflowDB, filters={"id": workflow_id, "user_id": user_id})
        return {"status": result.status, "message": "Workflow deleted successfully"}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.post("/workflows/run")
async def create_workflow_run(body: CreateWorkflowRunRequest, db=Depends(get_db)) -> Dict:
    """Create an ephemeral workflow run - returns temporary run_id"""
    try:
        run_id = str(uuid.uuid4())
        
        if body.workflow_id:
            workflow = db.get(WorkflowDB, filters={"id": body.workflow_id}, return_json=False)
            if not workflow.status or not workflow.data:
                raise HTTPException(status_code=404, detail="Workflow not found")
            
            workflow_config = workflow.data[0].config
            if isinstance(workflow_config, dict) and "metadata" not in workflow_config:
                workflow_config["metadata"] = {
                    "name": workflow_config.get("label") or "Workflow",
                    "description": workflow_config.get("description") or "",
                }
        elif body.workflow_config:
            workflow_config = body.workflow_config
            if isinstance(workflow_config, dict) and "metadata" not in workflow_config:
                workflow_config["metadata"] = {
                    "name": workflow_config.get("label") or "Workflow",
                    "description": workflow_config.get("description") or "",
                }
        else:
            raise HTTPException(status_code=400, detail="Either workflow_id or workflow_config must be provided")
        
        return {
            "status": True,
            "data": {
                "run_id": run_id,
                "workflow_config": workflow_config
            }
        }
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/workflows/library/steps")
async def list_workflow_steps(
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """Get available workflow steps"""
    try:
        steps: List[ComponentModel] = get_default_steps()
        return {"status": True, "data": steps}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e

 

# ==================== WebSocket Routes ====================

active_workflow_runs: Dict[str, Dict[str, Any]] = {}


class WorkflowWebSocketManager:
    """Manages WebSocket connections for workflow execution"""
    
    def __init__(self):
        self.connections: Dict[str, WebSocket] = {}
    
    async def connect(self, websocket: WebSocket, run_id: str) -> bool:
        try:
            await websocket.accept()
            self.connections[run_id] = websocket
            logger.info(f"Workflow WebSocket connected for run {run_id}")
            return True
        except Exception as e:
            logger.error(f"Failed to connect workflow WebSocket: {e}")
            return False
    
    async def disconnect(self, run_id: str):
        if run_id in self.connections:
            del self.connections[run_id]
        if run_id in active_workflow_runs:
            del active_workflow_runs[run_id]
        logger.info(f"Workflow WebSocket disconnected for run {run_id}")
    
    async def send_message(self, run_id: str, message: Dict[str, Any]):
        if run_id in self.connections:
            try: 
                await self.connections[run_id].send_json(serialize_for_json(message))
            except Exception as e:
                logger.error(f"Failed to send message to run {run_id}: {e}")
    
    async def execute_workflow(self, run_id: str, workflow_config: Dict[str, Any], initial_input: Any = None):
        try:
            # Normalize the workflow config structure for Workflow.load_component
            # Frontend sends config with nested config.steps/config.edges and metadata at top,
            # but Workflow.load_component expects steps/edges/metadata at top level.
            if isinstance(workflow_config, dict):
                # Extract the inner config if steps are nested under a "config" key
                inner_config = workflow_config.get("config", {})
                if isinstance(inner_config, dict) and "steps" in inner_config:
                    steps = inner_config.get("steps", [])
                    edges = inner_config.get("edges", [])
                    start_step_id = inner_config.get("start_step_id")
                    end_step_ids = inner_config.get("end_step_ids", [])
                else:
                    steps = workflow_config.get("steps", [])
                    edges = workflow_config.get("edges", [])
                    start_step_id = workflow_config.get("start_step_id")
                    end_step_ids = workflow_config.get("end_step_ids", [])

                # Build metadata if missing
                metadata = workflow_config.get("metadata")
                if not metadata:
                    metadata = {
                        "name": workflow_config.get("label") or workflow_config.get("name") or "Workflow",
                        "description": workflow_config.get("description") or "",
                    }

                normalized_config = {
                    "provider": workflow_config.get("provider") or "autogenstudio.workflow.Workflow",
                    "component_type": workflow_config.get("component_type") or "workflow",
                    "version": workflow_config.get("version") or 1,
                    "component_version": workflow_config.get("component_version") or 1,
                    "label": workflow_config.get("label") or "Workflow",
                    "description": workflow_config.get("description") or "",
                    "metadata": metadata,
                    "steps": steps,
                    "edges": edges,
                    "start_step_id": start_step_id,
                    "end_step_ids": end_step_ids,
                }
            else:
                normalized_config = workflow_config

            # Ensure each step's config has required fields (input_schema, output_schema) with defaults
            for step in normalized_config.get("steps", []):
                if isinstance(step, dict) and isinstance(step.get("config"), dict):
                    step_config = step["config"]
                    if "input_schema" not in step_config:
                        step_config["input_schema"] = {"type": "object", "properties": {}}
                    if "output_schema" not in step_config:
                        step_config["output_schema"] = {"type": "object", "properties": {}}

            # Use WorkflowConfig.model_validate to get clear Pydantic errors, then build workflow
            try:
                validated_config = WorkflowConfig.model_validate(normalized_config)
                workflow = Workflow._from_config(validated_config)
            except Exception as validation_error:
                logger.error(f"Config validation error: {validation_error}")
                raise
            runner = WorkflowRunner()
            
            try:
                async for event in runner.run_stream(workflow, initial_input):
                    event_data = event.model_dump()
                    event_data["run_id"] = run_id
                    await self.send_message(run_id, event_data)
                
            except Exception as workflow_error:
                await self.send_message(run_id, {
                    "type": "workflow_error",
                    "run_id": run_id,
                    "error": str(workflow_error),
                    "timestamp": datetime.now().isoformat()
                })
                
        except Exception as e:
            logger.error(f"Workflow execution error for run {run_id}: {e}")
            await self.send_message(run_id, {
                "type": "workflow_error", 
                "run_id": run_id,
                "error": f"Failed to execute workflow: {str(e)}",
                "timestamp": datetime.now().isoformat()
            })


workflow_manager = WorkflowWebSocketManager()


@router.websocket("/workflow/ws/{run_id}")
async def workflow_websocket(
    websocket: WebSocket,
    run_id: str,
    auth_manager=Depends(get_ws_auth_manager),
):
    """WebSocket endpoint for workflow execution"""
    
    try:
        connected = await workflow_manager.connect(websocket, run_id)
        if not connected:
            return
        
        logger.info(f"Workflow WebSocket connection established for run {run_id}")
        
        active_workflow_runs[run_id] = {
            "created_at": datetime.now(),
            "status": "connected"
        }
        
        raw_message = None
        while True:
            try:
                raw_message = await websocket.receive_text()
                message = json.loads(raw_message)
                
                if message.get("type") == "start":
                    logger.info(f"Received workflow start request for run {run_id}")
                    
                    workflow_config = message.get("workflow_config")
                    initial_input = message.get("input")
                    
                    if not workflow_config:
                        await websocket.send_json({
                            "type": "error",
                            "error": "workflow_config is required",
                            "timestamp": datetime.now().isoformat(),
                        })
                        continue
                    
                    asyncio.create_task(
                        workflow_manager.execute_workflow(run_id, workflow_config, initial_input)
                    )
                
                elif message.get("type") == "stop":
                    logger.info(f"Received workflow stop request for run {run_id}")
                    await websocket.send_json({
                        "type": "workflow_stopped",
                        "run_id": run_id,
                        "timestamp": datetime.now().isoformat(),
                    })
                    break
                
                elif message.get("type") == "ping":
                    await websocket.send_json({
                        "type": "pong", 
                        "timestamp": datetime.now().isoformat()
                    })
                
                else:
                    logger.warning(f"Unknown message type: {message.get('type')}")
                    
            except json.JSONDecodeError:
                logger.warning(f"Invalid JSON received: {raw_message or 'None'}")
                await websocket.send_json({
                    "type": "error", 
                    "error": "Invalid message format", 
                    "timestamp": datetime.now().isoformat()
                })
    
    except WebSocketDisconnect:
        logger.info(f"Workflow WebSocket disconnected for run {run_id}")
    except Exception as e:
        logger.error(f"Workflow WebSocket error: {str(e)}")
    finally:
        await workflow_manager.disconnect(run_id)


@router.get("/workflow/ws/status/{run_id}")
async def get_workflow_run_status(run_id: str):
    """Get status of an active workflow run"""
    if run_id not in active_workflow_runs:
        return {"status": False, "message": "Run not found"}
    
    run_info = active_workflow_runs[run_id]
    return {
        "status": True,
        "data": {
            "run_id": run_id,
            "created_at": run_info["created_at"].isoformat(),
            "status": run_info["status"]
        }
    }
