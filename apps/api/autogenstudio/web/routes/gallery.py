...
async def update_gallery_entry(
    request: Request, gallery_id: int, gallery_data: Gallery, db: DatabaseManager = Depends(get_db)
) -> Response:
...
@router.post("/")
async def create_gallery_entry(request: Request, gallery_data: Gallery, db: DatabaseManager = Depends(get_db)) -> Response:
    user_id = get_authenticated_user_id(request)
    gallery_data.user_id = user_id
...
@router.get("/")
async def list_gallery_entries(request: Request, db: DatabaseManager = Depends(get_db)) -> Response:
    user_id = get_authenticated_user_id(request)
    try:
...
async def get_gallery_entry(request: Request, gallery_id: int, db: DatabaseManager = Depends(get_db)) -> Response:
    user_id = get_authenticated_user_id(request)
...
async def delete_gallery_entry(request: Request, gallery_id: int, db: DatabaseManager = Depends(get_db)) -> Response:
    user_id = get_authenticated_user_id(request)
